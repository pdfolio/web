...
const GatherMain = ({ args }) => {
  ...
  const getAll = async () => {
    try {
      const data = await gatherApiNoToken(
        `/api/v1/gather` +
          `?page=${page}&size=${size}&keyword=${keyword}&category=${category}&skills=${skills}`,
        'GET',
      );
      console.log(data);
      setAll(data);
  ...
  return (
    <div>
      ...
      <div className="category-style">
        ...
        <Button
          className={`ct-button ${
            category === 'PROJECT' ? 'button-choice' : ''
          }`}
          color="primary"
      ...
      <div className="skill-style">
        <InputGroup className="search-bar">
          ...
          <InputGroupText addontype="append">
            <Button color="primary" onClick={onClickSearchHandler}>
              Search
            </Button>
          </InputGroupText>
        </InputGroup>
          ...
          <Card>
            <CardBody>
              {skillNames.map((skill, index) => (
                <Badge
                  key={index}
                  onClick={() => onClickSkillHandler(skill)}
                  className={
                    confirmSkills.includes(skill)
                      ? 'sill-badge-location skill-border'
                      : 'sill-badge-location'
                  }
                  color="info"
                >
            ...
            <Card className="my-2 ccard" key={c.id}>
              ...
              <CardBody>
                ...
                <CardText className="card-text-st ">
                  ...
                  &nbsp;&nbsp;&nbsp;
                  <span
                    className={c.category === 'PROJECT' ? 'borderItem' : ''}
                  >
                    <img
                      width={'28px'}
                      height={'28px'}
                      src="https://cdn-icons-png.flaticon.com/128/950/950232.png"
                    ></img>
                  </span>
                  <br></br>
                  <br></br>
              ...
              <CardFooter className="card-footer2">
                <span className="img-location2">
                  <img
                    width={'23px'}
                    height={'23px'}
                    src="https://cdn-icons-png.flaticon.com/128/2338/2338317.png"
                  ></img>
                </span>
                &nbsp;
                <span className="id-size">{c.memberName}</span>
                <span className="img-location3 side-size">
                  <img
                    width={'23px'}
                    height={'23px'}
                    src="https://cdn-icons-png.flaticon.com/128/4249/4249907.png"
                  ></img>{' '}
                  {c.commentCount}
                </span>
                <span className="img-location4 side-size">
                  <img
                    width={'23px'}
                    height={'23px'}
                    src="https://cdn-icons-png.flaticon.com/128/64/64945.png"
                  ></img>{' '}
                  {c.viewCount}
                </span>
              </CardFooter>
